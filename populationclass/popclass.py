...
def birth_death_event(population:Population):
    poplen              =  reduce( lambda x,y: x+y,list(population.typecount_dict.values()) )
    fitness_values      =  [*map(lambda individ: individ.fitness, population.population)]
    avg_fitness         =  np.sum(fitness_values)/poplen

    brate               =  ( avg_fitness )/( avg_fitness + poplen * 0.01 )
    drate               =  ( poplen * 0.01 ) / (avg_fitness + poplen*0.01)
...
with open(f'{outfilename}_(itern={itern}).csv', 'w', newline='') as file:
    writer = csv.writer(file)
    writer.writerows([t3,t5])