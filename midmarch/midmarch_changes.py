...
for _ in range (400):
    POPULATION.append(createIdividual(1))
...
for it in range(int(itern)):
    if it%100000 == 0:
        print(f"{it/100000}/30")
    t1.append(population_proper.typecount_dict[1])
    t6.append(population_proper.typecount_dict[6])
    brates.append(np.around(population_proper.brate,4))
    population_proper.birth_death_event()

with open(f'competition1/siminst_{siminst}_test.csv', 'w',newline='') as filein:
    writer = csv.writer(filein)
    writer.writerows([t1,t6,brates])
    print(f"Wrote instance 1 ")
